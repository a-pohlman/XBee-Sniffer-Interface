...
def time_ahead(current_time: list[int] = [12, 50, 30], future_time_ahead: list[int] = [1, 30, 00], format: int = 12) -> str:
        ...
        if format == 24: return f"{append_zero(new_time[0])}:{append_zero(new_time[1])}:{append_zero(new_time[2])}"
...
def convert_seconds(seconds:int) -> str:
    hour_float: float = seconds/3600
    hour_int: int = int(hour_float)

    minute_percentage: float = float(hour_float - hour_int)
    minute: int = int(round(60*minute_percentage))

    return f"{'' if hour_int < 1 else f"{hour_int} {'hour' if hour_int == 1 else 'hours'} "}{f"{int(seconds)} {'second' if seconds == 1 else 'seconds'}" if seconds < 60 else f"{minute} {'minute' if minute == 1 else 'minutes'}"}"
